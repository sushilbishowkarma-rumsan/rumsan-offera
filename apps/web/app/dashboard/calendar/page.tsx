...
// ── Determine what type of leave a user has on a specific date ──
...
export default function CalendarPage() {
  ...
  const { data: allLeaves = [], isLoading: leavesLoading } =
    useCalendarLeaveRequests();
  const { data: allWfh = [], isLoading: wfhLoading } = useCalendarWfhRequests(); // ← NEW
  ...
  // ← NEW: Filter WFH same way as leaves
  ...
  const stats = useMemo(() => {
    ...
    // ← Count WFH in "on leave today" stat
  ...
  function normalizeDate(dateStr: string): string {
    // Handles both "2026-03-06" and "2026-03-06T00:00:00.000Z"
    // Always returns "YYYY-MM-DD" in LOCAL terms (not UTC shifted)
    if (!dateStr) return '';
    const raw = dateStr.split('T')[0]; // "2026-03-06"
    return raw;
  }
  // ── UPDATED: returns leave type + day type for each user on a date ──
  function getMembersOnLeave(date: Date) {
    ...
    const results: {
      ...
      dayType: string; // FULL | FIRST_HALF | SECOND_HALF
    }[] = [];
    ...
    for (const u of filteredUsers) {
      ...
      const leave = filteredLeaves.find((l) => {
        ...
        // If per-day breakdown exists, check exact date match
        // If per-day breakdown exists, match exactly
        if (l.leaveDays?.length > 0) {
          return l.leaveDays.some((d) => normalizeDate(d.date) === dateStr);
        }
        // Otherwise check date range
        // Fallback: date range check (timezone-safe)
      ...
      if (leave) {
        // Get the specific day type for this date
        let dayType = 'FULL';
  ...
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(160deg,#fdf6ec 0%,#faf2e4 40%,#f5ede0 100%)',
      }}
    >
      <style>{`
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {user && user.role === 'EMPLOYEE' && user.department && (
            <div
              ...
              style={{ background: '#fff', border: '1.5px solid #ddc9a8' }}
            >
              <span className="text-sm" style={{ color: '#9a5c1a' }}>
                Your Department:
              </span>
              <Badge
                variant="secondary"
        ...
        {/* {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              {
                labelNp: 'आज अनुपस्थित',
                labelEn: 'On Leave/WFH Today',
              ...
              {
                labelNp: 'स्वीकृत बिदा',
                labelEn: 'Approved Leaves',
              ...
              <div
                key={labelEn}
                className="stat-card flex flex-col gap-0.5 rounded-2xl px-4 py-3"
                style={{
                  background: bg,
                  border: `1.5px solid ${border}`,
                  boxShadow: `0 4px 16px ${border}28`,
                }}
              >
                <p
                  className="cal-np text-[11px] font-semibold"
                  style={{ color }}
                >
                  {labelNp}
                </p>
                ...
                <p
                  className="cal-serif text-[28px] font-bold leading-tight mt-0.5"
                  style={{ color }}
                >
                  {value}
                </p>
            ...
            <div className="relative flex flex-wrap gap-2">
              {monthHolidays.map((h) => (
                <span
                  ...
                  style={{
                    background: h.isOptional ? '#fffbeb' : '#fef2f2',
                    border: `1px solid ${h.isOptional ? '#fde68a' : '#fca5a5'}`,
                    color: h.isOptional ? '#92400e' : '#991b1b',
                  }}
                >
                  <span
                    className="h-1.5 w-1.5 rounded-full"
                    style={{ background: h.isOptional ? '#f59e0b' : '#ef4444' }}
                  />
        ...
        <div className="flex flex-col xl:flex-row gap-5">
          ...
          <div
            ...
            style={{
              ...
              boxShadow: '0 8px 40px rgba(107,29,14,0.07)',
            }}
          >
            {/* Header Band */}
            <div
              ...
              style={{
                background:
                  'linear-gradient(135deg,#6b1d0e 0%,#9a3412 35%,#b45309 70%,#9a3412 100%)',
              }}
            >
              <div
            ...
            <div
              ...
              style={{
                background: '#fdf6ec',
                borderBottom: '1.5px solid #e7d5b8',
              }}
            >
            ...
            {isLoading ? (
              ...
              >
                {Array.from({ length: 35 }).map((_, i) => (
                  <Skeleton
                    ...
                    style={{ background: '#e8d5b0', minHeight: '80px' }}
                  />
                ))}
              </div>
            ) : (
              ...
              >
                ...
                {days.map((day) => {
                  ...
                  // Separate leave vs WFH members for dot colors
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        style={{
                          background: isToday ? '#6b1d0e' : 'transparent',
                          color: isToday
                            ? '#fff'
                            : isSat
                              ? '#dc2626'
                      ...
                      {holiday && (
                        <div className="flex items-center gap-0.5 mt-0.5">
                          ...
                          <span
                            ...
                            style={{ color: '#9a3412', maxWidth: '50px' }}
                          >
                            {holiday.name}
                          </span>
                        </div>
                      ...
                      {/* ── Avatar dots: leave (orange) + WFH (blue) ── */}
                            ...
                            .map(({ user: u, leaveType, dayType }) => {
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    background: isWfh ? '#dbeafe' : '#fef3c7',
                                    color: isWfh ? '#1d4ed8' : '#92400e',
                                    border: `1px solid ${isWfh ? '#93c5fd' : '#fcd34d'}`,
                                  }}
                                  title={`${u.name} — ${isWfh ? 'WFH' : dtConfig.label}`}
                                >
                                  {getInitials(u.name).slice(0, 1)}
                                  {/* Small dot showing day type for leave */}
                                  {!isWfh && dayType !== 'FULL' && (
                                    <span
                  ...
                  },
                  {
                    swatch: (
                      <div
                        ...
                        style={{ background: '#6b1d0e', color: '#fff' }}
                      >
                        T
                      </div>
                    ),
          ...
          {selectedDay ? (
            ...
            >
              ...
              <div
                className="relative overflow-hidden"
                style={{
                  background: 'linear-gradient(135deg,#6b1d0e,#9a3412,#b45309)',
                }}
              >
                <div
                          ...
                          <Avatar className="h-9 w-9 rounded-xl shrink-0">
                            <AvatarFallback
                              ...
                              style={{
                                background: isWfh ? '#dbeafe' : '#fef3c7',
                                color: isWfh ? '#1d4ed8' : '#92400e',
                                border: `1.5px solid ${isWfh ? '#93c5fd' : '#fcd34d'}`,
                              }}
                            >
                              {getInitials(u.name)}
                            </AvatarFallback>
              ...
              >
                ...
                <p className="text-[12px]" style={{ color: '#78350f' }}>
                  ...
                  {' of '}
                  <span className="font-bold" style={{ color: '#6b1d0e' }}>
