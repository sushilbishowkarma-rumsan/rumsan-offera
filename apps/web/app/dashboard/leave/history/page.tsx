...
// app/dashboard/leave/history/page.tsx  (or wherever your history page lives)
// REPLACE your existing LeaveHistoryPage with this file.
//
// Changes:
//   1. Added "Exceeded" third tab showing ExceededLeaveRequest records
//   2. All original Leave + WFH tabs unchanged
...
import { Skeleton } from "@/components/ui/skeleton";
import {
  CalendarPlus, Sun, Sunset, CalendarDays, Laptop, AlertTriangle,
} from "lucide-react";
import Link from "next/link";
import { useQuery } from "@tanstack/react-query";
import { api } from "@/lib/api";
import { EmployeeExceededPanel } from "@/components/exceeded-leave/employee-exceeded-panel";
import { useEmployeeExceededLeaves } from "@/hooks/use-exceeded-leave";

// ── Day type config (unchanged from original) ─────────────────────────────────
const DAY_TYPE_CONFIG: Record<string, {
  label: string; bg: string; color: string; border: string; icon: any;
}> = {
  FULL:        { label: "Full Day", bg: "#eef2ff", color: "#4f46e5", border: "#c7d2fe", icon: CalendarDays },
...
function LeaveDaysSummary({ req }: { req: any }) {
  ...
  const sorted = [...req.leaveDays].sort((a: any, b: any) => a.date.localeCompare(b.date));
  return (
    <div className="mt-1.5 flex flex-col gap-1">
      {sorted.map((d: any, i: number) => {
        const cfg = DAY_TYPE_CONFIG[d.dayType] ?? DAY_TYPE_CONFIG.FULL;
        ...
        return (
          <div key={d.id ?? i} className="flex items-center gap-2">
            <span className="text-[11px] w-28 shrink-0" style={{ color: "#64748b" }}>{label}</span>
...
export default function LeaveHistoryPage() {
  ...
  // Exceeded leaves count for tab badge
  const { data: exceededRequests = [] } = useEmployeeExceededLeaves(user?.id);

  const isLoading = leavesLoading || wfhLoading;

  // ── Active tab: "leave" | "wfh" | "exceeded" ─────────────────────────────
  const [activeTab, setActiveTab] = useState<"leave" | "wfh" | "exceeded">("leave");
  const [statusFilter, setStatusFilter] = useState("all");
  const [typeFilter, setTypeFilter] = useState("all");
  ...
  const tabs = [
    {
      ...
      activeStyle: { background: "#6366f1", color: "#ffffff", boxShadow: "0 2px 8px rgba(99,102,241,0.3)" },
    },
    {
      key: "wfh" as const,
      label: "WFH Requests",
      icon: Laptop,
      count: wfhRequests.length,
      activeStyle: { background: "#0ea5e9", color: "#ffffff", boxShadow: "0 2px 8px rgba(14,165,233,0.3)" },
    },
    {
      key: "exceeded" as const,
      label: "Exceeded",
      icon: AlertTriangle,
      count: exceededRequests.length,
      activeStyle: { background: "#ea580c", color: "#ffffff", boxShadow: "0 2px 8px rgba(234,88,12,0.3)" },
    },
  ];
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f8f9fc" }}>
      <div className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-1 rounded-xl p-1 self-start"
          style={{ background: "#ffffff", border: "1px solid #e2e8f0" }}>
          {tabs.map(({ key, label, icon: Icon, count, activeStyle }) => (
            <button
              key={key}
              onClick={() => setActiveTab(key)}
              className="flex items-center gap-2 rounded-lg px-4 py-2 text-[13px] font-semibold transition-all duration-150"
              style={activeTab === key ? activeStyle : { color: "#64748b" }}
            >
              <Icon className="h-3.5 w-3.5" />
              {label}
              {count > 0 && (
        ...
        {/* ── Filters (only for leave and wfh tabs) ── */}
        {activeTab !== "exceeded" && (
          <div
            className="flex flex-col gap-3 sm:flex-row rounded-2xl px-5 py-4"
            style={{ background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(15,23,42,0.05)" }}
        ...
        {/* ── EXCEEDED TAB — uses the shared component ── */}
        {activeTab === "exceeded" && user?.id && (
          <EmployeeExceededPanel employeeId={user.id} />
        )}
          ...
          <div className="flex flex-col rounded-2xl overflow-hidden"
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-16">
                ...
                <Link href="/dashboard/leave/request"
                  className="text-[12px] font-semibold" style={{ color: "#6366f1" }}>
                  Submit your first request →
                </Link>
              </div>
            ) : (
          ...
          <div className="flex flex-col rounded-2xl overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-4"
              ...
              <div>
                ...
                <p className="text-[11px] mt-0.5" style={{ color: "#94a3b8" }}>
                  {filteredWfh.length} records found
                </p>
              </div>
            </div>
