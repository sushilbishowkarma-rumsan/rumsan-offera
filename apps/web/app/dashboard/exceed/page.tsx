...
//  *
//  * HR Admin — Exceeded Leave Balances Page
//  * ────────────────────────────────────────
//  * Lists every employee with exceeded > 0 on any leave type.
//  *
//  * Features:
//  *   - Search by name / email / department
//  *   - Department filter dropdown
//  *   - Expandable per-employee cards with leave-type breakdown
//  *   - Refresh button
//  *   - "Clear All Exceeded" button → red confirmation modal → DELETE API call
//  *   - Empty state when no one has exceeded days
//  */
...
//   ShieldAlert,
// } from 'lucide-react';
// import { useAllExceededBalances, useClearAllExceeded } from '@/hooks/use-leave-balance';
...
//       bar: '#6366f1', badge: '#eef2ff', text: '#4f46e5',
//     }
//   );
// }
...
//           onClick={onCancel}
//           disabled={isClearing}
//           className="absolute top-4 right-4 rounded-lg p-1 transition-colors hover:bg-slate-100"
//           aria-label="Cancel"
//         >
...
//         {/* Title */}
//         <h2 className="text-center text-[17px] font-bold text-slate-900">
//           Clear All Exceeded Days?
//         </h2>

//         {/* Description */}
//         <p className="mt-2 text-center text-[13px] text-slate-500 leading-relaxed">
//           This will archive and reset exceeded leave days for{' '}
//           <span className="font-semibold text-slate-800">
//             {totalEmployees} employee{totalEmployees !== 1 ? 's' : ''}
//           </span>{' '}
//           totalling{' '}
//           <span className="font-semibold text-red-600">{totalDays} exceeded day{totalDays !== 1 ? 's' : ''}</span>.
//         </p>
...
//               Exceeded data will be saved to history before being cleared.
//             </p>
//             <p>Employees will no longer see the payroll deduction alert after this reset.</p>
//           </div>
//         </div>
...
//             style={{
//               background: isClearing ? '#fca5a5' : '#ef4444',
//               boxShadow: isClearing ? 'none' : '0 4px 12px rgba(239,68,68,0.35)',
//             }}
//           >
//             {isClearing ? (
//               <>
...
//         >
//           <p
//             className="text-[11px] font-semibold uppercase tracking-wider pt-3"
//             style={{ color: '#94a3b8' }}
//           >
...
//             const withinPct =
//               leaf.total > 0
//                 ? Math.min(100, Math.round((leaf.usedWithinQuota / leaf.total) * 100))
//                 : 100;
...
//                     style={{ width: `${withinPct}%`, background: '#ef4444' }}
//                   />
//                 </div>
...
//     data.forEach((row) => { if (row.employee.department) set.add(row.employee.department); });
...
//   // Stats drawn from the full (unfiltered) dataset — so the modal warning
//   // always shows the real total, not just the filtered subset.
//   const totalAffected     = filtered.length;
//   const totalExceededDays = filtered.reduce((s, r) => s + r.totalExceededDays, 0);

//   // Full-dataset totals for the confirmation modal
//   const allAffected    = data?.length ?? 0;
//   const allExceededSum = data?.reduce((s, r) => s + r.totalExceededDays, 0) ?? 0;

//   // ── Handle confirmed clear ───────────────────────────────────────────────
//   function handleClearConfirmed() {
//     clearExceeded(undefined, {
//       onSuccess: () => {
//         setShowConfirm(false);
//       },
//       onError: () => {
//         setShowConfirm(false);
//         // You can add a toast here if you have one in your project
//       },
//     });
//   }
...
//   return (
//     <>
//       {/* ── Confirmation modal (portal-like, rendered at top of tree) ── */}
//       <ConfirmClearModal
//         open={showConfirm}
//         totalEmployees={allAffected}
//         totalDays={allExceededSum}
...
//             {/* ── Action buttons: Refresh + Clear All ── */}
...
//               {/* Clear All — only shown when there is data to clear */}
...
//                   style={{
//                     background: isClearing ? '#fca5a5' : '#ef4444',
//                     boxShadow: isClearing ? 'none' : '0 2px 8px rgba(239,68,68,0.30)',
//                     opacity: isClearing ? 0.7 : 1,
//                   }}
//                   title="Archive and clear all exceeded days"
//                 >
//                   <Trash2 className="h-3.5 w-3.5" />
//                   Clear All Exceeded
//                 </button>
...
//               <span className="font-semibold">Manual clear:</span>{' '}
//               Clicking "Clear All Exceeded" archives current data to{' '}
//               <code className="bg-blue-100 rounded px-1">LeaveBalanceHistory</code> then
//               resets <code className="bg-blue-100 rounded px-1">exceeded = 0</code> for all
//               employees. Employees will no longer see the payroll deduction warning after this.
//             </p>
//           </div>
...
//                   {totalAffected} employee{totalAffected !== 1 ? 's' : ''}
//                 </span>
...
//                   : 'No exceeded balances right now'}
//               </p>
 ...
 * rumsan-offera/apps/web/app/dashboard/exceed/page.tsx
 *
 * HR Admin — Exceeded Leave Balances (v3)
 * ────────────────────────────────────────
 * Lists every employee with exceeded > 0 on any leave type.
 *
 * Features:
 ...
 *   - Department filter dropdown
 *   - Expandable per-employee cards with leave-type breakdown
 *   - Button row: Refresh | View History | Clear All Exceeded | Year-End Reset
 *   - "Clear All Exceeded" → red confirmation modal
 *   - "Year-End Reset" → orange confirmation modal (destructive, fresh balances)
 *   - Empty state when no one has exceeded days
 */
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={() => router.push('/dashboard/exceeded-history')}
                disabled={isBusy}
