...
import {
  Search, Eye, Sun, Sunset, CalendarDays, Clock, CheckCircle2,
  XCircle, Users, FileText, Laptop, Briefcase,
} from "lucide-react";
...
function RequestDetailDialog({ req, onClose }: { req: any; onClose: () => void }) {
  ...
  return (
    <Dialog open={!!req} onOpenChange={onClose}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-[15px] font-semibold flex items-center gap-2" style={{ color: "#0f172a" }}>
            Request Details
            <RequestTypeBadge type={isWfh ? 'wfh' : 'leave'} />
          </DialogTitle>
          <DialogDescription className="text-[12px]" style={{ color: "#64748b" }}>
            {req.employee?.name ?? req.employee?.email} · {isWfh ? 'Work From Home' : `${req.leaveType.charAt(0) + req.leaveType.slice(1).toLowerCase()} leave`}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: "#f8f9fc", border: "1px solid #e2e8f0" }}>
            <Avatar className="h-10 w-10 rounded-xl">
              <AvatarFallback className="rounded-xl text-[12px] font-bold" style={{ background: isWfh ? "#dbeafe" : "#eef2ff", color: isWfh ? "#1d4ed8" : "#4f46e5" }}>
                {getInitials(req.employee?.name ?? "?")}
              </AvatarFallback>
            </Avatar>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: isWfh ? "Request Type" : "Leave Type", value: isWfh ? "Work From Home" : req.leaveType.charAt(0) + req.leaveType.slice(1).toLowerCase() },
              { label: "Total Days", value: req.totalDays },
              { label: "Start Date", value: formatDate(req.startDate) },
              { label: "End Date", value: formatDate(req.endDate) },
              { label: "Manager", value: req.manager?.name ?? "—" },
...
export default function AdminRequestsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState("all");
  const [typeFilter, setTypeFilter] = useState("all");
  const [requestTypeFilter, setRequestTypeFilter] = useState("all"); // ✅ NEW: leave/wfh filter
  const [selectedRequest, setSelectedRequest] = useState<any>(null);

  // ✅ COMBINE both leave and WFH requests
  const allRequests = useMemo(() => {
    const leaves = leaveRequests.map((r: any) => ({ ...r, requestType: 'leave' }));
    const wfh = wfhRequests.map((r: any) => ({ ...r, requestType: 'wfh' }));
    return [...leaves, ...wfh].sort((a, b) => 
  ...
  // ── Leave types (from leave requests only)
  const leaveTypes = useMemo(() => [...new Set(leaveRequests.map((r: any) => r.leaveType))], [leaveRequests]);
  ...
  const filtered = useMemo(() => {
    return allRequests.filter((req: any) => {
      ...
      // ✅ Leave type filter (only applies to leave requests)
      const matchType = typeFilter === "all" || 
        (req.requestType === 'leave' && req.leaveType === typeFilter);
      
      // ✅ Request type filter (leave vs wfh)
      const matchRequestType = requestTypeFilter === "all" || req.requestType === requestTypeFilter;
      
      return matchSearch && matchStatus && matchType && matchRequestType;
    });
      ...
      <div className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
            ...
            {isLoading ? (
              ...
              todayRequests.map((req: any) => {
                ...
                return (
                  ...
                  >
                    <Avatar className="h-8 w-8 shrink-0 rounded-lg">
                      <AvatarFallback className="rounded-lg text-[10px] font-bold" style={{ background: isWfh ? "#dbeafe" : "#eef2ff", color: isWfh ? "#1d4ed8" : "#4f46e5" }}>
                        {getInitials(req.employee?.name ?? "?")}
                      </AvatarFallback>
                    </Avatar>
        ...
        >
          ...
          >
            ...
            {/* ✅ UPDATED: Search + filters (added request type filter) */}
            <div className="flex flex-col gap-2 sm:flex-row">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5" style={{ color: "#94a3b8" }} />
                <input
              ...
              {/* ✅ NEW: Request Type Filter (Leave/WFH) */}
              <Select value={requestTypeFilter} onValueChange={setRequestTypeFilter}>
                <SelectTrigger className="h-9 w-full sm:w-32 rounded-xl text-[13px]" style={{ background: "#f8f9fc", border: "1px solid #bfc2c7", color: "#475569" }}>
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <Avatar className="h-7 w-7 shrink-0 rounded-lg">
                              <AvatarFallback className="rounded-lg text-[9px] font-bold" style={{ background: isWfh ? "#dbeafe" : "#eef2ff", color: isWfh ? "#1d4ed8" : "#4f46e5" }}>
                                {getInitials(req.employee?.name ?? "?")}
                              </AvatarFallback>
                            </Avatar>
                            <div className="min-w-0">
                        ...
                        <td className="px-5 py-3.5">
                          {isWfh ? (
                            <span className="text-[11px]" style={{ color: "#64748b" }}>Work From Home</span>
                          ) : (
                            <span className="inline-flex items-center rounded-lg px-2 py-0.5 text-[10px] font-semibold" style={{ background: "#eef2ff", border: "1px solid #c7d2fe", color: "#4f46e5" }}>
                              {req.leaveType.charAt(0) + req.leaveType.slice(1).toLowerCase()}
                            </span>
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5">
                            <span className="text-[12px] font-medium" style={{ color: "#1e293b" }}>{req.totalDays}</span>
                            {!isWfh && req.isHalfDay && <HalfDayBadge period={req.halfDayPeriod} />}
                          </div>
                        </td>
