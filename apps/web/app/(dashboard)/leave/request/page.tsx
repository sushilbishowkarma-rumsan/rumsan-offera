...
export default function LeaveRequestPage() {
  ...
  const selectedBalance = useMemo(
    () =>
      balanceSummary.find(
        (b: LeaveBalanceSummary) => b.leaveType === leaveType,
      ),
  ...
  const dateRangeLabel = useMemo(() => {
    if (useMultiDay) {
      const filled = leaveDays
        .filter((d) => d.date !== '')
        .sort((a, b) => a.date.localeCompare(b.date));
      if (filled.length === 0) return '';
      if (filled.length === 1)
        return formatDateRangeLabel(filled[0].date, filled[0].date);
  ...
  const setErr = useCallback(
    (key: string, msg: string) => setFieldErrors((p) => ({ ...p, [key]: msg })),
    [],
  );
  ...
  const handleWfhStartDateChange = useCallback(
    (val: string) => {
      const check = isSelectableDate(val, holidayDateSet);
      if (!check.valid) {
        setWfhStartError(check.reason!);
        return;
      }
      setWfhStartError('');
      setWfhStartDate(val);
  ...
  const handleWfhEndDateChange = useCallback(
    (val: string) => {
      const check = isSelectableDate(val, holidayDateSet);
      if (!check.valid) {
        setWfhEndError(check.reason!);
        return;
      }
      setWfhEndError('');
      setWfhEndDate(val);
  ...
  // ── MULTI-DAY HELPERS ──
  const addLeaveDay = () =>
    setLeaveDays((p) => [...p, { date: '', dayType: 'FULL' }]);
  ...
  const updateLeaveDay = (i: number, field: keyof LeaveDay, value: string) => {
    if (field === 'date') {
      ...
      if (!check.valid) {
        setLeaveDayErrors((p) => ({ ...p, [i]: check.reason! }));
        return;
      }
      const usedDates = leaveDays
        .map((d, idx) => (idx !== i ? d.date : null))
        .filter(Boolean);
      if (usedDates.includes(value)) {
        setLeaveDayErrors((p) => ({
          ...p,
          [i]: 'This date is already added — remove the duplicate',
        }));
        return;
      }
      setLeaveDayErrors((p) => {
    ...
    setLeaveDays((p) =>
      p.map((d, idx) => (idx === i ? { ...d, [field]: value } : d)),
    );
  };
  ...
  const handleLeaveSubmit = (e: React.FormEvent) => {
    ...
    const parseResult = leaveFormSchema.safeParse({
      ...
      startDate,
      endDate,
      leaveDays: useMultiDay ? leaveDays : undefined,
      reason,
    });
    ...
    if (leaveTotalDays === 0) {
      setErr('startDate', 'Selected date range contains no business days');
      return;
    }
    if (!managerId) {
      setErr('managerId', 'No manager assigned to your account');
      return;
    }
    const limitMsg = checkLeaveTypeLimit(leaveType, leaveTotalDays);
    if (limitMsg) {
      setLimitError(limitMsg);
      return;
    }
    ...
    const duplicate = findDuplicateLeave(
      allLeaveRequests,
      effectiveStart,
      effectiveEnd,
    );
    if (duplicate) {
      ...
      setDuplicateError(
        `You already have a ${duplicate.leaveType} request from ${fmt(duplicate.startDate)} to ${fmt(duplicate.endDate)} (${duplicate.status}). Please check your existing requests.`,
      );
      return;
    }

    const wfhConflict = (allWfhRequests ?? []).find((req) => {
      ...
      const checkStart = new Date(effectiveStart.split('T')[0] + 'T00:00:00');
      const checkEnd = new Date(effectiveEnd.split('T')[0] + 'T00:00:00');
      return checkStart <= reqEnd && checkEnd >= reqStart;
    });
    ...
    if (wfhConflict) {
      ...
      setDuplicateError(
        `You have a WFH request from ${fmt(wfhConflict.startDate)} to ${fmt(wfhConflict.endDate)} (${wfhConflict.status}) that overlaps these dates. Cancel the WFH request first.`,
      );
      return;
    }
  ...
  const handleWfhSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    const parseResult = wfhFormSchema.safeParse({
    ...
    if (!parseResult.success) {
      ...
      setWfhFieldErrors(errs);
      return;
    }
    if (wfhTotalDays === 0) {
      setWfhFieldErrors({
        startDate: 'Selected range contains no business days',
      });
      return;
    }
    if (!managerId) {
      setWfhFieldErrors({ managerId: 'No manager assigned to your account' });
      return;
    }
    const wfhConflictResult = findDuplicateWfh(
      allWfhRequests,
      allLeaveRequests,
      wfhStartDate,
      wfhEndDate,
    );
    if (wfhConflictResult) {
      const { conflict, source } = wfhConflictResult;
      const fmt = (d: string) =>
        new Date(d.split('T')[0] + 'T00:00:00').toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
      ...
      const typeLabel =
        source === 'leave'
          ? `a ${(conflict as any).leaveType ?? 'leave'} leave`
          : 'a WFH';
      setWfhDuplicateError(
        `You already have ${typeLabel} request from ${fmt(conflict.startDate)} to ${fmt(conflict.endDate)} (${conflict.status}) that overlaps these dates.`,
      );
      return;
    }
    setWfhFieldErrors({});
              ...
              {(['leave', 'wfh'] as const).map((m) => (
                <button
                  ...
                  style={{
                    ...
                    boxShadow:
                      mode === m ? '0 1px 6px rgba(79,70,229,0.2)' : 'none',
                  }}
                >
                  {m === 'leave' ? (
                    <>
          ...
          <form onSubmit={handleLeaveSubmit}>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
              ...
              <div className="lg:col-span-2 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        {policiesLoading ? (
                          ...
                          <div
                            className="flex h-10 items-center rounded-xl px-3 text-[12px]"
                            style={{
                              background: '#fef9ec',
                          ...
                          <>
                            ...
                            >
                              <SelectTrigger
                                className="rounded-xl text-[13px] h-10 w-full"
                        ...
                        {leaveDays.map((day, index) => (
                          <div key={index} className="flex flex-col gap-1">
                            <div className="flex items-center gap-2">
                              <span
                                className="flex items-center justify-center h-5 w-5 shrink-0 rounded-full text-[10px] font-bold"
                                style={{
                            ...
                            {leaveDayErrors[index] && (
                              <div className="ml-7">
                                <FieldError msg={leaveDayErrors[index]} />
                              </div>
                            )}
                          </div>
                        ))}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              style={{
                                ...inputStyle,
                                cursor: isHalfDay ? 'not-allowed' : 'pointer',
                              }}
                            />
                ...
                <div
                  className="rounded-2xl overflow-hidden -mt-3.5"
                  style={{
                    background: C.card,
                    border: `1px solid ${C.cardBorder}`,
                    boxShadow: '0 1px 3px rgba(0,0,0,0.04)',
              ...
              <div className="lg:col-span-1 flex flex-col gap-4 lg:sticky lg:top-6">
                ...
                {selectedBalance && leaveImpact ? (
                  <div
                    className="rounded-2xl overflow-hidden"
                    style={{
                      background: leaveImpact.willExceed
                        ? C.warnBg
                        : C.successBg,
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        borderBottom: `1px solid ${leaveImpact.willExceed ? C.warnBorder : C.successBorder}`,
                      }}
                    >
                      {leaveImpact.willExceed ? (
                        ...
                        <Info
                          className="h-3.5 w-3.5 shrink-0"
                          style={{ color: C.successIcon }}
                        />
                      )}
                      <span
                        className="text-[10px] font-bold uppercase tracking-wider"
                        style={{
                          color: leaveImpact.willExceed
                            ? C.warnText
                            : C.successText,
                        }}
                      >
                    ...
                    <div className="p-4 flex flex-col gap-2">
                      ...
                      {[
                        ...
                        {
                          label: 'Remaining',
                          value: leaveImpact.currentRemaining,
                        },
                      ].map(({ label, value }) => (
                        <div
                          key={label}
                          ...
                          style={{
                            borderBottom: `1px solid ${leaveImpact.willExceed ? '#fed7aa44' : '#bbf7d044'}`,
                          }}
                        >
                          <span
                            className="text-[12px]"
                      ...
                      {leaveImpact.willExceed && (
                        ...
                        >
                          ...
                          <div
                            ...
                            style={{ borderTop: `1px solid ${C.errorBorder}` }}
                          >
                            <span
                              className="text-[12px] font-bold"
                              style={{ color: '#7f1d1d' }}
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: C.textMuted }}
                    >
                      Select a leave type to see your balance and payroll
                      impact.
                    </p>
                  </div>
                )}
              ...
              <div className="lg:col-span-2 flex flex-col gap-4">
                ...
                >
                  ...
                  <div className="p-5 flex flex-col gap-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          value={wfhStartDate}
                          onChange={(e) =>
                            handleWfhStartDateChange(e.target.value)
                          }
                          min={todayStr}
                          className="h-10 w-full rounded-xl px-3 text-[13px] outline-none"
                          style={{ ...inputStyle, cursor: 'pointer' }}
                        />
                        <FieldError
                          msg={wfhStartError || wfhFieldErrors.startDate}
                        />
                      </div>
                      <div>
                        <SectionLabel>End Date *</SectionLabel>
                        <input
                          ...
                          value={wfhEndDate}
                          onChange={(e) =>
                            handleWfhEndDateChange(e.target.value)
                          }
                        ...
                        />
                        <FieldError
                          msg={wfhEndError || wfhFieldErrors.endDate}
                        />
                      </div>
                    </div>
                    ...
                    {wfhFieldErrors.managerId && (
                      <FieldError msg={wfhFieldErrors.managerId} />
                    )}
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        type="submit"
                        disabled={createWfh.isPending}
                        className="flex-1 sm:flex-none sm:min-w-[160px] rounded-xl py-2.5 px-6 text-[13px] font-bold text-white disabled:opacity-50"
                        style={{
                          background: `linear-gradient(135deg, ${C.sky} 0%, #0284c7 100%)`,
                          cursor: createWfh.isPending
                            ? 'not-allowed'
                            : 'pointer',
