...
export default function CalendarPage() {
  ...
  // ── All roles see all users ──
  const visibleUsers = useMemo(() => allUsers, [allUsers]);

  // ── All roles see all approved leaves ──
  const visibleLeaves = useMemo(
    () => allLeaves.filter((l) => l.status === 'APPROVED'),
    [allLeaves],
  );

  // ── All roles see all approved WFH ──
  const visibleWfh = useMemo(
    () => allWfh.filter((w) => w.status === 'APPROVED'),
    [allWfh],
  );

  // ── Department filter available to all roles ──
  ...
  const filteredUsers = useMemo(() => {
    if (departmentFilter !== 'all')
      return visibleUsers.filter((u) => u.department === departmentFilter);
    return visibleUsers;
  }, [visibleUsers, departmentFilter]);
  ...
  // ── All roles can navigate to user profiles ──
      ...
      <div className="cal-sans max-w-7xl mx-auto flex flex-col gap-5 p-4 -mt-5 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Department filter — visible to ALL roles */}
...
//   //   if (user.role === 'HRADMIN' || user.role === 'MANAGER' || user.role === 'EMPLOYEE') return allUsers;
//   //   // if (user.role === 'EMPLOYEE' && user.department)
//   //     // return allUsers.filter((u) => u.department === user.department);
//   //   return allUsers;
//   // }, [allUsers, user]);
//   const visibleUsers = useMemo(() => {
//   // If everyone sees everyone, just return the full list.
//   // We only check for 'user' to ensure the person is logged in.
//   return user ? allUsers : []; 
// }, [allUsers, user]);

// // ── Visible Leaves ──
// const visibleLeaves = useMemo(() => {
//   // Just return the full list. No filtering by status or department.
//   return allLeaves;
// }, [allLeaves]);
